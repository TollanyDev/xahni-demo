 ...
 * Then 4 botones .x-btn en .x-actions:
 *   - "+ Nueva tarea" → `showView('tareas-prof')`.
 *   - "↑ Subir recurso" → `showView('recursos')`.
 *   - "⚠️ Alumnos en riesgo" (.x-btn--danger) con badge count.
 *   - "📐 Escala de evaluación" → `showView('gestion-academica')`.
 * Edge:
 *   - **Path LEGACY**: las views `tareas-prof`, `recursos`, `gestion-academica`
 *     fueron retiradas como standalone en C9. `showView` cae a navigation
 *     legacy que ahora redirige al hub correspondiente.
 *   - El espejo `_hubInicioBandaActionsProf` (slice C9) reemplazó las
 *     llamadas `showView` por `hubShellSwitchTab(...)` para entrar
 *     directamente a la tab del hub.
 *   - badge usa `x-btn__badge` (clase canónica). nRiesgo 0 también muestra
 *     "0" (decisión consciente: visibilidad sobre minimalismo).
 */
function _buildProfDashQA(nRiesgo) {
    return `<div class="x-actions" style="margin-bottom:16px">
        <button class="x-btn x-btn--primary" onclick="showView('tareas-prof')"><span>＋</span> Nueva tarea</button>
        <button class="x-btn x-btn--ghost" onclick="showView('recursos')"><span>↑</span> Subir recurso</button>
        <button class="x-btn x-btn--danger" onclick="showView('gestion-academica')">⚠️ Alumnos en riesgo <span class="x-btn__badge">${nRiesgo}</span></button>
        <button class="x-btn x-btn--ghost" onclick="showView('gestion-academica')">📐 Escala de evaluación</button>
    </div>`;
}
...
function _buildProfDashGrupos(data) {
    ...
    return `<div class="card">
        <div class="card-header">
            ...
            <button class="card-action" onclick="showView('gestion-academica')">Ver detalle →</button>
        </div>
...
function _buildProfDashRiesgo(data) {
    ...
    const content = riesgo.length === 0
        ...
        : riesgo.map(a => {
            ...
            return `<div class="x-list-row x-list-row--link" onclick="showView('gestion-academica')">
