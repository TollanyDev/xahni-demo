 ...
 */
async function _tpGuardarTarea(publicar) {
  ...
  if (_tpEditId) {
    // FIX: antes solo se mutaba el objeto en TP_TAREAS (memoria local del
    // profe) con Object.assign — el cambio nunca se escribía en Firestore
    // ni en DEMO_TAREAS, así que el alumno seguía viendo la versión vieja
    // de la tarea (o ninguna, si el profe editaba un borrador recién creado).
    const payload = {
      id:           _tpEditId,
      titulo,
      materiaId:    materia,
    ...
    try {
      const actualizada = await DataService.saveTarea(payload);
      // En prod, el .set(merge:true) solo pega en Firestore; replicamos
      // en DEMO_TAREAS local para que la sesión actual quede consistente
      // sin esperar un reload (mismo patrón que en la rama de creación).
      if (DataService.prodMode) {
        const idx = DEMO_TAREAS.findIndex((t) => t.id === _tpEditId);
        if (idx >= 0) DEMO_TAREAS[idx] = { ...DEMO_TAREAS[idx], ...actualizada };
      }
      ...
      showToast("No se pudo actualizar la tarea. Intenta de nuevo.", "error");
      return;
    }
  } else {
    // FIX: antes solo se hacía push a TP_TAREAS (copia local en memoria del
    // profesor) y nunca se llamaba a DataService.saveTarea, por lo que la
    // tarea jamás llegaba a Firestore ni a DEMO_TAREAS (fuente de verdad que
    // consume el alumno en getTareasAlumno). Ahora se persiste vía
    // DataService, que en demo hace push a DEMO_TAREAS y en prod escribe en
    // la colección 'tareas' de Firestore.
    const payload = {
      titulo,
      materiaId:    materia,
      grupoId,
      profesorId:   APP.user.id,
      tipo, puntos,
    ...
    try {
      const nueva = await DataService.saveTarea(payload);
      // En prod, saveTarea solo escribe a Firestore (no muta DEMO_TAREAS
      // localmente); lo empujamos aquí para que la sesión actual del
      // profesor y el resto de builders vean el cambio sin esperar un reload.
      if (DataService.prodMode && !(DEMO_TAREAS || []).some((t) => t.id === nueva.id)) {
        DEMO_TAREAS.push(nueva);
      }
      TP_TAREAS.push(_tpFromDemo(nueva));
      showToast(publicar ? "Tarea publicada" : "Borrador guardado", "success");
    } catch (err) {
      console.error("Error al guardar tarea:", err);
      showToast("No se pudo guardar la tarea. Intenta de nuevo.", "error");
      return; // no cerramos el modal para no perder lo capturado por el profe
    }
  }
