 ...
 */
async function ejecutarAccionRecurso() {
  ...
  // FIX: antes se mutaba solo _recData (copia local de sesión del profesor)
  // sin llamar nunca a DataService.saveRecurso, por lo que el recurso jamás
  // llegaba a Firestore ni a DEMO_RECURSOS (fuente que lee el alumno) — mismo
  // bug que tareas.js.
  try {
    if (_recEditId) {
      const actualizado = await DataService.saveRecurso({
        id:        _recEditId,
        titulo:    nombre,
        materiaId: materia,
        ...( _recArchivoNombre ? { tipo: _recTipoDesdeNombre(_recArchivoNombre) } : {} ),
      });
      if (DataService.prodMode) {
        const dIdx = DEMO_RECURSOS.findIndex((x) => x.id === _recEditId);
        if (dIdx >= 0) DEMO_RECURSOS[dIdx] = { ...DEMO_RECURSOS[dIdx], ...actualizado };
      }
      const r = _recData.find((x) => x.id === _recEditId);
      if (r) Object.assign(r, actualizado);
      showToast(`"${nombre}" actualizado`, "success");
    } else {
      if (!_recArchivoNombre) { showToast("Selecciona un archivo", "error"); return; }
      const nuevo = await DataService.saveRecurso({
        titulo:      nombre,
      ...
      if (DataService.prodMode && !(DEMO_RECURSOS || []).some((x) => x.id === nuevo.id)) {
        DEMO_RECURSOS.push(nuevo);
      }
      _recData.push(nuevo);
    ...
    showToast("No se pudo guardar el recurso. Intenta de nuevo.", "error");
    return;
  }
