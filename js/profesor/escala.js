...
function seleccionarEscalaMat(matId) {
  ...
  if (_hubCtx && matId) {
    _escalaMat = matId;
    const gmKey = (typeof profMatGrupoKey === "function")
      ? profMatGrupoKey(matId, _hubCtx.grupoId || APP.profGrupoActivo)
      : `${matId}_${_hubCtx.grupoId || APP.profGrupoActivo}`;
...
function _persistEscalaState(esc) {
  ...
  gruposFinales.forEach(grupoId => {
    const escalaIdValue = grupoId
      ? (typeof escalaId === "function" ? escalaId(_escalaMat, grupoId, _escalaParcial) : `${_escalaMat}_${grupoId}_${_escalaParcial}`)
    ...
    if (typeof DEMO_ESCALAS !== "undefined") {
      const idx = DEMO_ESCALAS.findIndex(e => e.id === escalaIdValue);
