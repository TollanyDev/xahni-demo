 ...
 * Then orquesta data provider gigante (~115 LOC):
 *   1. Resuelve `_profDataBase(uid)` (shared del rol) → null → empty shape.
 *   2. **Enriquece materias**:
 *      - color paleta `_GEST_MAT_PALETTE` + grad `_GEST_MAT_GRADS` (índice).
 *      - periodoInfo + parciales[].
 *      - `parcialActivoNum` cascada: parcial.estado="activo" > primer parcial > 1.
 *      - `escalasPorParcial` dict `{1: esc, 2: esc, 3: esc}` por parcial,
 *        lookup en DEMO_ESCALAS por id compuesto `${m.id}_${grupoPrincipal}_${num}`.
 *      - `escalaActiva` = escala del parcial activo (alias para conveniencia).
 *   3. **Enriquece alumnos** (1 entry por par estudiante×materia):
 *      - dedupe via Set `${estId}_${matId}` (alumno en varios grupos NO duplica).
 *      - prom calculado desde tareas + entregas reales (filter por uid y
 *        calificación no-null).
 *      - tareasDetalle con título+fecha+estado (entregada/pendiente).
 *      - sort tareas pendientes al final.
 ...
 *   4. cals shell (legacy `actualizarCal` consumes): `{id, nombre, materiaId,
 *      p1: null, p2: null, p3: null}` por alumno.
 * Edge:
 *   - **_profDataBase null** → empty `{materias: [], alumnos: [], cals: []}`.
 *   - **Dedupe alumnos cross-grupo**: si alumno está en 2 grupos de misma
 *     materia, solo 1 entry (decisión: 1 cardId por par estudiante×materia).
 *     Grupo asignado = el primero encontrado en iteración.
 *   - **Función PURA** respecto a inputs (lookup + map + filter; no muta
 *     globals).
 *   - Función gigante (~115 LOC) — el data hub central del módulo.
 *   - Deuda post-Supabase: vista materializada `gestion_alumnos_view` con
 *     joins + aggregates SQL-side.
 */
function getProfGestionData(uid) {
  ...
  // Materias con paleta UI + periodo (semanas por parcial)
  const materias = materiasSrc.map((m, i) => {
    // Resuelve grupoPrincipal al PRIMER grupo de la materia QUE EXISTA en
    // DEMO_GRUPOS. Bug previo: el primer entry de m.grupos era legacy
    // ("ISC-3A") inexistente en runtime, lo cual hacía que getPeriodoDeGrupo
    // retornara null y escalasPorParcial quedara vacío.
    ...
    const parcialActivoNum = parciales.find(p => p.estado === "activo")?.num
        ?? parciales[0]?.num
        ?? 1;
    const grupoPrincipal = grupoPrincipalIni;
    // Escala POR parcial (dict {1: esc, 2: esc, 3: esc}) — cada parcial tiene
    // su propia escala. Se consume desde el miniperfil cuando se selecciona P1/P2/P3.
    const escalasPorParcial = {};
    const _ESC = Array.isArray(typeof DEMO_ESCALAS !== "undefined" ? DEMO_ESCALAS : null) ? DEMO_ESCALAS : null;
    if (_ESC) {
      parciales.forEach(p => {
        const id = `${m.id}_${grupoPrincipal}_${p.num}`;
  ...
  // Alumnos: una entrada por par (estudiante, materia); prom derivado de entregas reales
  const alumnos = [];
  const seen    = new Set();
 ...
 *      consumed por `buildTareasProfesor` (en tareas.js) al entry.
 *   2. Navega via `showView("tareas-prof")`.
 * Edge:
 *   - **`showView` ausente** → no-op (hook persiste para futuro navigation).
 *   - **Pattern hook cross-vista**: idéntico al consumido en
 *     `buildTareasProfesor` (sesión 10 doc). Single-use + delete.
 *   - **Exportado en window** (onclick inline).
 *   - Función IMPURA (window state + nav).
 *   - **Path legacy**: `showView("tareas-prof")` ya no es la ruta canonical
 *     (post-shell-rework C9). Redirige al hub-shell via navigation legacy.
 *     Deuda C9-leftover.
 */
function _gestionVerTareas(uid, matId) {
    window._tareasFiltro = { uid, materiaId: matId };
    if (typeof showView === "function") showView("tareas-prof");
}
  ...
  _gestionData.alumnos.forEach(a => {
    ...
    Object.keys(obj).forEach(parcKey => {
      ...
      DEMO_PROGRESO_ESCALA[key] = DEMO_PROGRESO_ESCALA[key] || {
        uid,
        escalaId:    `${a.materia}_${a.grupo}_${parcN}`,
        materiaId:   a.materia,
        grupoId:     a.grupo,
        parcialNum:  parcN,
        criterios:   [],
